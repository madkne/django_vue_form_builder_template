...
/************************************* */
type CommandName = 'watch' | 'install' | 'make' | 'rebuild-all' | 'rebuild-page-components';
...
export async function main(): Promise<number> {
   ...
   let res = await ARG.define<CommandName, CommandArgvName>([
      ...
      {
         name: 'rebuild-page-components',
         description: 'rebuild all pages that have specific component',
         alias: 'rpc',
         implement: async () => await rebuildPagesByComponent(),
         argvs: [
            // {
            //    name: 'make-component',
            //    alias: 'c',
...
async function install() {
   ...
   await OS.shell(`sudo npm install -g @vue/cli@4.5.15 -f`);
   await OS.shell(`sudo npm i -g @vue/cli-service-global@4.5.15 -f`);
...
/************************************* */
async function rebuildPagesByComponent() {
   // =>get component name
   let componentName = await IN.input(`Enter component name: (like form_builder)`);
   // =>detect all pages
   let pages = await detectAllPages();
   let rebuildPagesCount = 0;
   // =>iterate on all pages and filter them
   for (const page of pages) {
      // =>read .dfc.json file
      let dfc = JSON.parse(fs.readFileSync(path.join(page.path, '.dfc.json')).toString());
      // =>search on components
      if (dfc['components'] && dfc['components'].find(i => i == componentName)) {
         page['must_build'] = true;
         rebuildPagesCount++;
      }
   }
   LOG.info(`${rebuildPagesCount} pages detected that rebuild...`);
   let pageIndex = 0;
   while (true) {
      let page = pages[pageIndex];
      // =>check for rebuild
      if (!page || !page['must_build']) {
         pageIndex++;
         if (pageIndex >= pages.length) {
            break;
         }
...
async function loadAllConfig(): Promise<ConfigsObject> {
   ...
   let ConfigVariables: { name: ConfigVariableKey; default?: any }[] = [
      // {
      //    name: 'git_url',
      //    default: 'https://192.168.31.227/ifc/talent.git',
      // },
      // {
...
async function rebuildPageOrComponent(pagePath: string, pageName: string, type: 'page' | 'component' = 'page') {
   ...
   `);
      ...
      if (type === 'page') {
         // await OS.shell(`node ${await OS.cwd()}/node_modules/@vue/cli/bin/vue.js build -t lib -n ${pageName} -d .tmp/.dist .tmp/${pageName}.middle.vue`, pagePath);
         ...
         // =>copy umd js map file of page to assets
         fs.copyFileSync(path.join(pagePath, '.tmp', '.dist', `${pageName}.umd.min.js.map`), path.join(pageStaticDirPath, pageName + '.umd.min.js.map'));
